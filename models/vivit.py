...
from algorithms.counting import CountedLinear
...
class FactorizedViViT(ExtendedModule):
    ...
    def __init__(
        ...
        spatial_config["drop_using_rollout"] = drop_using_rollout
        ...
        self.patches_y = input_h // tubelet_shape[1]
        self.patches_x = input_w // tubelet_shape[2]
    ...
    def _forward_view(self, x):
        ...
        if not (self.drop_using_rollout or self.spatial_model.backbone.force_compute_rollout):
            ...
            for t in range(x.shape[1]):
                first_frame = t == 0
                x_out, heatmap, tome_clusters_maps, active_tokens_ind, metric_t = \
                    self.spatial_model(x[:, t], first_frame, t_min1_heatmaps, t, t_min1_metric)
                x_tmp.append(x_out)
                t_min1_heatmaps = heatmap.clone().detach()
...
class ViViTSubModel(ExtendedModule):
    ...
    def forward(self, x, first_frame=False, t_min1_heatmaps=None, time_step=None, t_min1_metric=None):
        ...
        if self.backbone.drop_using_rollout or self.backbone.force_compute_rollout:
            x, heatmaps, tome_clusters_maps, active_tokens_ind, _, _, metric_t = \
                self.backbone(x, first_frame, t_min1_heatmaps, time_step, t_min1_metric)
        else:
            x = self.backbone(x)
        x = self.layer_norm(x)
        ...
        x = x[:, 0]
        if self.backbone.drop_using_rollout or self.backbone.force_compute_rollout:
            return x, heatmaps, tome_clusters_maps, active_tokens_ind, metric_t
        else:
            return x
