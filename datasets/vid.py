...
ROOT_DATA_PATH = '/stage/algo-datasets/DB/CBI'
...
class VID(Dataset):
    ...
    def __init__(
        ...
        constructor.
        :param sampling_period: an integer indicating that every
        k'th frame should be sampled in the video. By default set
        to None (sample all frames from the video)
        """
        ...
        self.video_info = self._get_videos_info(location, split, sampling_period)
    ...
    @staticmethod
    def _get_videos_info(location, split, sampling_period):
        ...
        if sampling_period is not None:
            for vid_ind in range(len(videos_info)):
                vid_meta = videos_info[vid_ind]
                vid_orig_frames = vid_meta['frames']
                n_orig = len(vid_orig_frames)
                vid_sampled_frames = [vid_orig_frames[i] for i in range(n_orig) if i % sampling_period == 0]
                vid_meta['frames'] = vid_sampled_frames
        return videos_info
